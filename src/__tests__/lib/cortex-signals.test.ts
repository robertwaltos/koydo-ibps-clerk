...
import { emitPrismSignal } from "@/lib/ibps-clerk/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "ibps-clerk" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "ibps-clerk" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "ibps-clerk" });
